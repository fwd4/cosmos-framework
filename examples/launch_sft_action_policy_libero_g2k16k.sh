...
# (128) matching the paper / issue-#50 ~2.7-passes regime, on HSDP 2x8 (16 GPUs).
...
# The TOML pins shard=8/replicate=2 (16 ranks) and grad_accum=1; this wrapper
# pins max_samples_per_batch=8 (a dataloader_train field, set via Hydra opt) so
# 8 x 16 x 1 = 128. See action_policy_libero_repro_g2k16k.toml.
#
...
#
# Usage (2 nodes x 8 GPU = HSDP 2x8): bash examples/launch_sft_action_policy_libero_b128.sh

TOML_FILE="examples/toml/sft_config/action_policy_libero_repro_g2k16k.toml"
...
TAIL_OVERRIDES=(
    dataloader_train.max_samples_per_batch=64
